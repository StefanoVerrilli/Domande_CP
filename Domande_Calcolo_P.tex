...
\color{black} \item Qual è la differenza tra Calcolo Parallelo in ambiente Distribuito \emph{(DM)} e in ambiente condiviso \emph{(SM)}?
...
\color{red} \item In che modo la non esatta divisibilità del numero di elementi per il numero di processori inficia negativamente per il calcolo dello \textit{speed-up}?
\color{red} \item Quale rapporto esiste tra il tempo di comunicazione $t_{com}$ e il tempo di svolgimento di un operazione floating point $t_{calc}$?
...
\color{red} \item Cosa comporta la clausola \textit{lastprivate} della libreria OpenMP e con che costrutto viene utilizzato?
\color{red} \item Perché il nucleo computazionale del prodotto matrice-vettore con la prima strategia di suddivisione viene definito del tipo \textit{embarassing parallel}?
\color{red} \item Perché è necessario per lo speed-up scalato tener traccia anche della dimensione \textit{n} del problema?
\color{red} \item Nella clausola \underline{scheduling} della libreria OpenMP per il costrutto parallel, quando è preferibile utilizzare la keyword \textit{dyncamic} invece di \textit{static}?
\color{red} \item Quali sono i costrutti \underline{worksharing} della libreria OpenMP? Sapresti definirne i loro differenti utilizzi?
...
\color{red}\item Come è possibile ottenere il rapporto tra numero di elementi \textit{n} e numero di processori \textit{p} tale che il nostro problema sia scalabile?
